package com.cumulocity.opcua.mock.configuration;
...
public class EventWatcher implements Closeable {
    ...
    private void logEvent(Object event, CountDownLatch countDownLatch) {
        if (event.getClass().getCanonicalName().contains("cumulocity")) {
            log.info("Event arrived " + event.getClass().getSimpleName() + " countDownLatch " + countDownLatch);
        }
    }
}
